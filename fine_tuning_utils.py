...
def load_models_from_stable_diffusion_checkpoint_vae(v2, ckpt_path, dtype=None):
  checkpoint = load_checkpoint_with_text_encoder_conversion(ckpt_path)
  state_dict = checkpoint["state_dict"]
  if dtype is not None:
    for k, v in state_dict.items():
...
def load_models_from_stable_diffusion_checkpoint_novae(v2, ckpt_path, dtype, device):
  checkpoint = load_checkpoint_with_text_encoder_conversion(ckpt_path)
  state_dict = checkpoint["state_dict"]
  if dtype is not None:
    for k, v in state_dict.items():
      if type(v) is torch.Tensor:
        state_dict[k] = v.to(device, dtype)
